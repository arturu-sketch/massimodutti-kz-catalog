...
def massimo_high_price(kzt: float) -> int:
    return int(kzt * MASSIMO_HIGH_PRICE_MARKUP / MASSIMO_HIGH_PRICE_DIVISOR)
...
def retail_price(kzt: float | int | None, brand: str | None = None) -> int | None:
    if kzt is None:
        return None
...
def parse_price(raw, brand: str | None = None) -> tuple[float | None, int | None]:
    if raw is None:
        return None, None
    try:
        kzt = int(raw) / 100
        return kzt, retail_price(kzt, brand)
    except Exception:
        return None, None
